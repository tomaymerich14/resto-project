...
class Trainer():
    ...
    def set_pipeline(self, model):
        ...
        categorical_features = [
            'jour', 'service', 'match_happening', 'match_happening_cl',
            'match_edf', 'roland_garros', 'fashion_week', 'vacances_paris'
        ]

        numerical_features = [
            'temp', 'feels_like', 'temp_min', 'temp_max', 'wind_speed',
            'clouds_all', 'jour_de_sem', 'jour_du_mois', 'mois_de_annee',
            'sem_de_annee', 'jour_annee', 'moyen_7_services',
            'moyen_31_services', 'moyenne_3der_j&service'
        ]
        ordinal_features = [
...
if __name__ == "__main__":
    ...
    preproc_data_d2 = pd.read_csv('../raw_data/preproc_data_d2_v2.csv')
    preproc_data_d16 = pd.read_csv('../raw_data/preproc_data_d16_v2.csv')

    X_d2 = preproc_data_d2.drop(columns=["CA_TTC",'CA_HT','TVA','date'])
    y_d2 = preproc_data_d2.CA_TTC
    X_d16 = preproc_data_d16.drop(columns=["CA_TTC", 'CA_HT', 'TVA', 'date'])
